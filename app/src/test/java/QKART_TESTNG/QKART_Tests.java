...
public class QKART_Tests {
    ...
    public void TestCase02(@Optional("testUser") String TC2_Username,
    ...
    public void TestCase03(String TC3_ProductNameToSearchFor) throws InterruptedException {
        ...
        assertFalse(searchResults.size() == 0,
                "There were no results for the given search string - "
                        + TC3_ProductNameToSearchFor);
        ...
        for (WebElement webElement : searchResults) {
            ...
            if (isSizeChartExists) {
                ...
                if (isSizeChartOpenSuccess) {
                    ...
                    Boolean isChartContentMatching = result.validateSizeChartContents(
                            expectedTableHeaders, expectedTableBody, driver);
                    assertTrue(isChartContentMatching,
    ...
    public void TestCase07(String TC7_ListOfProductsToAddToCart) throws InterruptedException {
        ...
        assertTrue(status,
                "Verify that insufficient balance error is thrown when the wallet balance is not enough: Fail");
        homePage.PerformLogout();
    }

    /*
     * Verify insufficient balance message is shown when order price is more than wallet balance
     */
    ...
    @Test(description = "Verify that a product added to a cart is available when a new tab is added",
            priority = 10, dependsOnMethods = {"TestCase10"}, groups = {"Regression_Test"})
    ...
    public void TestCase12(String TC12_ProductNameToSearch, String TC12_AddresstoAdd)
        ...
        assertFalse(status,
                "Clicking on the 'Buy now' button in the ad should redirect the main page");
